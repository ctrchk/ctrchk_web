...
(function () {
  ...
  // SPA readiness helper for injected scripts
  window.onPWAReady = function(fn) {
    if (window.CTRCHK_IS_STANDALONE) {
      setTimeout(fn, 1);
    } else {
      if (document.readyState !== 'loading') fn();
      else document.addEventListener('DOMContentLoaded', fn);
  ...
  function initPWAShell() {
    ...
    window.addEventListener('popstate', () => {
      navigateTo(window.location.pathname + window.location.search, { pushState: false });
    });

    window.navigateTo = navigateTo;
  }

  // Global state for map initialisation
  ...
  async function fetchPage(path) {
    ...
    try {
      ...
      let content;
      if (normalizedPath === '/') content = doc.querySelector('#app-home');
  ...
  function prefetchPWAPages() {
    ...
    paths.forEach(path => {
      if (path !== current) {
        setTimeout(() => fetchPage(path), 1200);
      }
    });
  }
