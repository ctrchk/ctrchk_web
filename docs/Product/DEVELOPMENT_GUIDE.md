...
### 1.2 維護現狀與工程制約
目前專案處於**極少開發者長期維護（甚至單人維護）** 的狀態。這是一個極為重要的工程制約條件。
因此，本專案的生命線是：**「保持簡單，保持容易維護，決不為了炫技而增加系統複雜度。」**

當你需要引入一個新工具、撰寫一段新邏輯、或是引入第三方依賴時，請務必反問自己以下三個問題：
1.  這段程式碼在三年後，如果沒有任何文檔，其他開發者（或 AI Agent）能否在 5 分鐘內完全看懂？
...
*   **業務用途**：在騎士騎行過程中，提供工業風的大字體 HUD，展示當前時速、累計里程、累計時間、已省車資（綠色交通指標）、中途站 Stop Pills 的橫向滾動報站。
*   **主要檔案**：
...
*   **業務用途**：將用戶的騎士卡一鍵添加至 Apple Wallet 或 Google Wallet，展示級別頭銜、滾動累積里程與專屬卡面。
*   **主要檔案**：
...
*   **業務用途**：基於 365 天滾動里程更新騎士等級與保級狀態；在騎行結束時，審查軌跡 GPX。
*   **主要檔案**：
...
*   **黑金主題下的強對比原則 (Critical Contrast Rule)**：
    ...
    *   **必須嚴格顯示為：黑底（`#000000 !important`）搭配金黃字/金黃背景（`#d4af37 !important`）**。這是防止陽光反射下騎士失明、保證騎行安全的強制規定。
...
PWA 是 CTRC HK 提供給騎士的終極體驗載體。PWA 的生命力來自其穩定與離線工作的能力。
