...
1. **查看 Vercel 部署日誌**：
   ...
   - 查找 `/api/register` 或 `/api/oauth` 的錯誤訊息
...
3. **檢查網路請求**：
   - 開發者工具 → Network 標籤
   - 嘗試註冊或登入
   - 查看 `/api/register` 或 `/api/oauth` 的請求和回應
...
### Supabase API Key 安全性

您可能注意到代碼中包含了 Supabase 的 API Key (anon key)。這是正常且安全的做法，因為：

1. **Anon Key 的設計**：Supabase 的 anonymous key 是專門設計用於客戶端代碼的公開金鑰
2. **權限控制**：此金鑰的權限受到 Supabase Row Level Security (RLS) 政策的嚴格控制
3. **用途限制**：此金鑰只能用於 Google OAuth 認證流程，無法直接訪問或修改資料庫
...
1. **雙重認證系統**：
   - Supabase 僅用於 Google OAuth 認證
   - 用戶資料儲存在 Vercel Postgres（完全獨立的資料庫）
...
3. **JWT Token**：
   - 使用您設定的 JWT_SECRET 簽署所有 token
   - Token 具有有效期限制

4. **環境變數**：
   - 敏感資訊（如 JWT_SECRET、資料庫連線）儲存在 Vercel 環境變數中
   - 這些資訊不會出現在客戶端代碼中
...
1. **定期更新密鑰**：
   - 定期在 Supabase 中輪換 API Key
   - 定期更新 JWT_SECRET

2. **設定 Supabase RLS**（如果直接使用 Supabase 資料庫）：
   - 由於本系統使用 Vercel Postgres 而非 Supabase 資料庫，RLS 不適用
   - 但如果未來改用 Supabase 資料庫，請務必設定 RLS 政策

3. **監控異常活動**：
   - 定期檢查 Vercel 的 Function Logs
   - 注意異常的登入嘗試
