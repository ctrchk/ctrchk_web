...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (action === 'get-managed-routes') {
      try {
        ...
        return res.status(200).json({ routes: rows.map(r => ({ ...r, route_id: `${r.dept}-${r.route_number}`, stops: parseJsonSafe(r.stops, []), rewards: parseJsonSafe(r.rewards, {}), tags: parseJsonSafe(r.tags, []), gpx: parseJsonSafe(r.gpx, []) })) });
      } catch (e) { return res.status(500).json({ message: 'Failed' }); }
    }
    ...
    try {
      ...
      const users = await query(`SELECT u.*, COALESCE(gp.level, 1) AS cyclist_level, COUNT(DISTINCT ch.id) AS ride_count, COALESCE(SUM(ch.distance_km), 0) AS total_distance FROM users u LEFT JOIN user_game_profile gp ON gp.user_id = u.id LEFT JOIN cycling_history ch ON u.id = ch.user_id ${where} GROUP BY u.id, gp.level ORDER BY u.created_at DESC LIMIT $1 OFFSET $2`, params);
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'upsert_managed_route') {
        await query(`INSERT INTO routes (dept, route_number, start_station_id, end_station_id, type, stops, rewards, alias, bg_color, estimated_minutes, unlock_type, unlock_value, tags, gpx, length_text, updated_at) VALUES ($1,$2,$3,$4,$5,$6::jsonb,$7::jsonb,$8,$9,$10,$11,$12,$13::jsonb,$14::jsonb,$15,NOW()) ON CONFLICT (dept, route_number) DO UPDATE SET start_station_id=EXCLUDED.start_station_id, end_station_id=EXCLUDED.end_station_id, type=EXCLUDED.type, stops=EXCLUDED.stops, rewards=EXCLUDED.rewards, alias=EXCLUDED.alias, bg_color=EXCLUDED.bg_color, estimated_minutes=EXCLUDED.estimated_minutes, unlock_type=EXCLUDED.unlock_type, unlock_value=EXCLUDED.unlock_value, tags=EXCLUDED.tags, gpx=EXCLUDED.gpx, length_text=EXCLUDED.length_text, updated_at=NOW()`, [b.dept, b.route_number, b.start_station_id, b.end_station_id, b.type, JSON.stringify(b.stops || []), JSON.stringify(b.rewards || {}), b.alias, b.bg_color, b.estimated_minutes, b.unlock_type, b.unlock_value, JSON.stringify(b.tags || []), JSON.stringify(b.gpx || []), b.length_text]);
        return res.status(200).json({ success: true });
    }
    ...
    if (action === 'get_game_profile') {
        const u = await query(`SELECT u.*, COALESCE(gp.level,1) as level, COALESCE(gp.xp,0) as xp, COALESCE(gp.coins,0) as coins, COALESCE(gp.mileage_km_365,0) as mileage_km_365, LOWER(COALESCE(gp.mileage_rank,'bronze')) as mileage_rank FROM users u LEFT JOIN user_game_profile gp ON gp.user_id=u.id WHERE u.id=$1`, [b.user_id]);
    ...
    if (action === 'set_game_stats') {
        ...
        await query(`INSERT INTO user_game_profile (user_id, level, xp, coins, mileage_km_365, mileage_rank, updated_at) VALUES ($1, COALESCE($2,1), COALESCE($3,0), COALESCE($4,0), COALESCE($5,0), COALESCE($6,'bronze'), NOW()) ON CONFLICT (user_id) DO UPDATE SET level=COALESCE($2, user_game_profile.level), xp=COALESCE($3, user_game_profile.xp), coins=COALESCE($4, user_game_profile.coins), mileage_km_365=COALESCE($5, user_game_profile.mileage_km_365), mileage_rank=COALESCE($6, user_game_profile.mileage_rank), updated_at=NOW()`, [b.user_id, lvl, b.xp, b.coins, b.mileage_km_365, b.mileage_rank]);
