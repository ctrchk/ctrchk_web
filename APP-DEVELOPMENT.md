...
- **全新 20 級積分系統** (`database-schema.sql`, `api/_db.js`, `dashboard.html`):
  ...
  - **稱號晉升機制**：Lv1–5 入門車手 → Lv6–15 初階車手 → Lv16–30 進階車手 → Lv31–50 資深車手 → Lv51–75 精英車手 → Lv76+ 頂尖車手
...
|------|------|---------|---------|
| Lv.1 | 入門車手 | 入門路線（2 條） | — |
| Lv.6 | 初階車手 | 新增 1 條路線 | 100 里程幣 |
| Lv.16 | 進階車手 | 新增 2 條路線 | 200 里程幣 |
| Lv.31 | 資深車手 | 特別挑戰路線 | 500 里程幣 |
| Lv.51 | 精英車手 | 全路線解鎖 | 1000 里程幣 |
...
| 資源 | 說明 | 如何在 App 中使用 |
|------|------|-----------------|
| JWT 認證 | `POST /api/login`、`POST /api/oauth` | App 登入後存儲 JWT token，後續 API 調用帶上 Bearer token |
...
INSERT INTO level_config (level, xp_required, coins_reward, title_zh, title_en) VALUES
  ...
  (10,   4000, 1000, '資深車手',   'Urban Legend')
